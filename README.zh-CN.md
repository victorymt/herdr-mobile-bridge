...
`http://127.0.0.1:8787`。首次访问时，在登录页输入电脑上通过
`node src/launcher.js pair` 生成的一次性配对码。浏览器会保持已认证会话；来源为 HTTPS 时，
还可以通过 **开启提醒** 按钮申请推送权限。

## 手机端登录与配对

手机登录页默认使用一次性配对码。电脑上在项目目录执行：

```bash
node src/launcher.js pair
```

命令要求 Bridge 正在运行，并通过本机 owner token 生成 8 位数字码。配对码只在终端显示，
5 分钟内有效且只能使用一次；它不会写入配置、日志、URL 或二维码。手机输入配对码后会换取
当前已有的短期浏览器会话。Bridge 重启后配对码会失效。

也可以在登录页切换为“使用访问令牌”，输入 `node src/launcher.js token` 输出的长期令牌。
...
Bridge 接收终端状态事件后会先将通知任务写入本地有界队列，再异步发送到 Web Push 服务。
网络故障、超时、`429` 和 `5xx` 会在 5 分钟有效窗口内按退避重试；Bridge 重启后会继续未完成任务。
失效订阅（`404`/`410`）会自动删除。通知设置弹层会显示本设备待处理和最近投递状态。
